...
public class CycleTimeMetricsServiceTest {
    ...
    void should_get_cycle_time_metrics_given_merged_on_branch_delivery_settings() throws SymeoException {
        ...
        final Date oldestCreationDate = DateHelper.stringToDate("2021-12-01");
        final List<PullRequestView> currentPullRequestViews =
                List.of(PullRequestView.builder().id(faker.animal().name())
                                .creationDate(oldestCreationDate)
                                .head(faker.ancient().god()).build(),
                        PullRequestView.builder().id(faker.animal().name())
                                .creationDate(DateHelper.stringToDate("2021-12-15"))
                                .head("main").build());
        ...
        when(expositionStorageAdapter.readAllCommitsForTeamIdAfterStartDate(teamId, oldestCreationDate))
        ...
        final List<PullRequestView> currentPullRequestViews =
                List.of(PullRequestView.builder().id(faker.animal().name())
                        ...
                        PullRequestView.builder()
                                ...
                                .id(faker.animal().name()).head("main").build());
        final List<PullRequestView> previousPullRequestViews =
                List.of(PullRequestView.builder().id(faker.animal().name()).head(faker.ancient().god()).build(),
                        PullRequestView.builder().id(faker.animal().name()).head("staging").build()
                );
