...
import io.symeo.monolithic.backend.domain.bff.port.out.TeamStandardStorage;
...
class TeamGoalService implements TeamGoalFacadeAdapter {
    ...
    @Override
    public Optional<TeamGoal> getOptionalTeamGoalForTeamIdAndTeamStandard(final UUID teamId,
                                                                          final TeamStandard teamStandard) throws SymeoException {
        final List<TeamGoal> teamGoals = teamGoalStorage.readForTeamId(teamId);
        return teamGoals
                .stream().filter(teamGoal -> teamGoal.getStandardCode().equals(teamStandard.getCode()))
                .findFirst();
